...
fn create_git_worktree_in_slot(
    ...
    let is_empty = fs::read_dir(slot_path)
        .context(slot_path)?
        .next()
        .is_none();
    if is_empty {
...
async fn attach_workspace(
    ...
    let (workspace, mut locked_wc, checkout_commit) = if has_leftover_state {
        ...
        let workspace = command.load_workspace_at(destination_path, workspace_command.settings())?;
    ...
    if let Some(git_repo) = crate::git_util::open_workspace_git_repo(&workspace, tx.repo())? {
        ...
        let actual_head_target = match git_repo.head_id() {
            Ok(id) => jj_lib::op_store::RefTarget::normal(
                jj_lib::backend::CommitId::from_bytes(id.as_bytes()),
            ),
            // Unborn HEAD.
