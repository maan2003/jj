...
workspace. Workspaces generated by `jj workspace add --detached` or
`jj workspace add --pool` use `ws-<id prefix>` names, and the same `ws-<id
prefix>` form can be used as a revset symbol for the workspace working-copy
commit. Use `<name>@<remote>` to refer to a remote-tracking tag / bookmark.
